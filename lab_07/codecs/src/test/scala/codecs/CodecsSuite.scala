...
class CodecsSuite
  ...
  // TODO individual
  test ("a 'Author' value should be encoded as a JSON object (1pt)") {
    val author = Author("Oscar", "Wilde")
    val json = Json.Obj(Map("name" -> Json.Str("Oscar"), "surname" -> Json.Str("Wilde")))
    val encoder = summon[Encoder[Author]]
    assertEquals(encoder.encode(author), json)
  }

  test ("it is possible to encode and decode authors (4pts)`") {
